...
const Header = () => {
  ...
  return (
    <header className="sticky top-0 z-50 transition-colors duration-300 bg-neutral-surface dark:bg-neutral-800 shadow-navbar">
        ...
        <div className="items-center hidden space-x-6 md:flex">
          {navLinks.map((link) => (
            <Link
              key={link.label}
              to={link.to}
              className="transition-colors duration-200 text-neutral-textSecondary dark:text-neutral-300 hover:text-brand-primary dark:hover:text-brand-accent"
            >
              {link.label}
            </Link>
          ))}
          <button
            ...
            className="p-2 transition-colors rounded-lg bg-neutral-200 dark:bg-neutral-700 hover:bg-neutral-300 dark:hover:bg-neutral-600"
      ...
      {isMobileMenuOpen && (
        <div className="absolute left-0 right-0 z-30 py-4 shadow-lg md:hidden top-full bg-neutral-surface dark:bg-neutral-800">
          <div className="container flex flex-col space-y-3">
            {navLinks.map((link) => (
              <Link
                key={`mobile-${link.label}`}
                to={link.to}
                className="block px-3 py-2 text-base font-medium transition-colors duration-200 rounded-md text-neutral-textPrimary dark:text-neutral-200 hover:bg-neutral-100 dark:hover:bg-neutral-700 hover:text-brand-primary dark:hover:text-brand-accent"
