...
import { useDarkMode } from '../../hooks/useDarkMode'; // Assuming useDarkMode hook path
...
const Header = () => {
  ...
  return (
    <header className="sticky top-0 z-50 w-full transition-colors duration-300 bg-neutral-surface dark:bg-neutral-800 shadow-navbar">
      <nav className="container flex items-center justify-between px-4 py-3 mx-auto">
        <Link to={ROUTES.HOME} className="flex items-center" aria-label="RocVille Media House Homepage">
          <img
            src={logoSrc}
            alt="RocVille Media House Logo"
            className="w-auto h-10 transition-opacity duration-300 ease-in-out hover:opacity-80" // Adjusted logo height
          />
          <span className="ml-2 text-2xl font-bold text-brand-primary dark:text-brand-primary-dark">
            RocVille
          </span>
        </Link>
      ...
      {isMobileMenuOpen && (
        <div className="absolute left-0 right-0 z-30 py-2 shadow-lg md:hidden top-full bg-neutral-surface dark:bg-neutral-800 ring-1 ring-black ring-opacity-5">
          ...
          </div> {/* Closing div for mobile menu */}
        </div>
      )}
    </header>
  );
...
Header.propTypes = {
  // No props are currently defined for Header.
  // This is a placeholder for future props or for documentation purposes.
};
